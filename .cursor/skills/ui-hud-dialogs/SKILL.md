...
- 小地图：`UI_SmallMapUV_Panel` / `UISmallMapUvPanel` / `UIId.SmallMap`。烘焙图钉玩家圆心、ΔXZ 摆怪标；海滩/肉鸽切 `MapBakeData_*`（SO 必须挂当前 `MapBakeData` 脚本 GUID）。`PlayerMapIcon` 整预制体随主相机 `YawTransform` 转。敌人 `EnemyAgentRegistry.Live` + 图标池。旧 RT 面板 `UI_SmallMap_Panel` 保留不用。`EnemyMapIcon` 与玩家图标同结构。
