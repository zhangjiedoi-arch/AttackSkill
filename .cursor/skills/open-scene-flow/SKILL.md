...
分镜面板推进 → 登录 / 选性别门闩 → 选角 Timeline → 加载 `GameScene`。有进度档则 `GameBoot.Continue`，否则 `NewGame`。性别写入 `LocalAccountStore`，供 Party 组装漂泊者男女。
...
6. 连接：磁盘有 `game_progress.json` 则 `GameBoot.Continue`，否则 `NewGame`。新开局先 F6 删档。
