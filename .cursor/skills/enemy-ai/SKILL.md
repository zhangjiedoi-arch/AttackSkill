...
- **肉鸽批量刷怪**：每波在玩家 10m 半径内随机落点同时生成（默认 8–16）。场上上限 `30 + 5*(Level-1)`，封顶 100（`RouGeLikeFlowController.MaxAliveNow`）。走 `EnemyObjectPool`；点会夹在平面内，并避开贴身/已有怪。传送后 BGM 切 `drone`，并打开 `UIBattleTimePanel` 3 分钟获救倒计时（剩余写入存档；结算后为 0）。场上上限 Inspector 可调（默认 30+5/级 cap100）。Bootstrap 找不到 `RouGeLikePlane` 会打 Error。
